...
const setupAudioListeners = () => {
  ...
  els.player.addEventListener('playing', () => { if(isPlaying) updateUIAsPlaying(); });
...
const updateUIAsPlaying = () => {
    ...
    if(els.visualizer) { els.visualizer.classList.remove("hidden"); els.visualizer.classList.add("playing-audio"); }
    if(els.track) els.track.innerText = "Señal Digital"; // SOLO "SEÑAL DIGITAL"
    startTimer();
};
...
const setPlayingState = (playing) => {
  ...
  if (playing) { updateUIAsPlaying(); if ('mediaSession' in navigator) navigator.mediaSession.playbackState = 'playing'; } 
  else {
    if(els.status) { els.status.innerText = "PAUSADO"; els.status.classList.remove("live"); }
    if(els.badge) els.badge.style.display = "none";
    if(els.visualizer) { els.visualizer.classList.remove("playing-audio"); els.visualizer.classList.add("hidden"); }
    if(els.track) els.track.innerText = "Pausado";
    stopTimer(); if ('mediaSession' in navigator) navigator.mediaSession.playbackState = 'paused';
  }
  renderList(); 
};
...
const updateMediaSession = () => { if ('mediaSession' in navigator) { navigator.mediaSession.metadata = new MediaMetadata({ title: currentStation.name, artist: currentStation.country + ' · ' + currentStation.region, album: 'Satelital Wave Player v8.0', }); navigator.mediaSession.setActionHandler('previoustrack', () => skipStation(-1)); navigator.mediaSession.setActionHandler('nexttrack', () => skipStation(1)); navigator.mediaSession.setActionHandler('play', () => { els.player.play(); setPlayingState(true); }); navigator.mediaSession.setActionHandler('pause', () => { els.player.pause(); setPlayingState(false); }); } };
