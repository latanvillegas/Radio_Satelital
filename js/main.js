// =======================
// ULTRA CORE v4.1 (LOGIC CONTROLLER)
// =======================
...
const init = () => {
  els = {
    ...
    // NUEVO: Selector de botones de vista
  ...
  // 2. FORZAR REINICIO (Limpieza de caché de formulario)
...
const setTheme = (themeName) => {
  ...
  // Lógica para pintar la barra de estado del navegador en móviles
  const metaTheme = document.querySelector('meta[name="theme-color"]');
  if(metaTheme) {
    if(themeName === "amoled") metaTheme.setAttribute("content", "#000000");
    else if(themeName === "white") metaTheme.setAttribute("content", "#ffffff"); // BLANCO PURO
...
const setupListeners = () => {
  ...
  // --- LÓGICA DE GRID (NUEVO) ---
  if(els.viewBtns) {
    els.viewBtns.forEach(btn => {
      btn.addEventListener("click", () => {
        hapticFeedback('light');
        // 1. Gestionar clases visuales del botón
        els.viewBtns.forEach(b => b.classList.remove("active"));
        btn.classList.add("active");
        
        // 2. Aplicar clase al contenedor de la lista
        const mode = btn.dataset.view;
        els.list.className = "station-grid"; // Reset clase base
        
        if(mode === "2") els.list.classList.add("grid-2");
        if(mode === "4") els.list.classList.add("grid-4");
        // Si es 'auto', no añadimos nada y el CSS usa su default inteligente
      });
    });
  }
};
