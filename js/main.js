...
const init = () => {
  ...
  // Asume que defaultStations ya está cargado desde stations.js
  const customStations = JSON.parse(localStorage.getItem("ultra_custom") || "[]");
  // Asegura que stations use la variable global defaultStations de stations.js
...
const togglePlay = () => {
  // Si no hay emisora seleccionada, inicia la primera por defecto (arranque en frío)
  if (!currentStation) {
...
// LÓGICA DE SALTO CORREGIDA (Siguiente / Anterior)
...
const setupListeners = () => {
  ...
  // Conexión de los nuevos botones de navegación
