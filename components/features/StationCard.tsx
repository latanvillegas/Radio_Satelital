...
export default function StationCard({ station, onPlay, onToggleFav }: { station: Station; onPlay?: (s: Station) => void; onToggleFav?: (s: Station) => void }) {
  ...
  const { currentStation, isPlaying, playbackStatus, togglePlay } = usePlayer()
  ...
  const handlePrimary = () => playing ? togglePlay() : onPlay?.(station)

  return <motion.article layout initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} whileHover={{ y: -2 }} onClick={handlePrimary}
    ...
    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handlePrimary() } }} aria-label={playing ? `Pausar ${station.name}` : `Escuchar ${station.name}`}>
    ...
    <div className="flex gap-3 items-center">
      ...
      <div className="min-w-0 flex-1"><h3 className="text-[15px] font-bold text-white truncate">{station.name}</h3><p className="text-xs text-zinc-400 truncate mt-0.5">{location}</p><p className="text-[11px] text-zinc-600 truncate mt-1">{genre || station.country || 'Radio en vivo'}</p></div>
      <div className="flex flex-col items-center gap-1.5 shrink-0">
        ...
        <button type="button" onClick={(e) => { e.stopPropagation(); handlePrimary() }} className="w-9 h-9 rounded-full grid place-items-center text-black shadow-lg transition-transform hover:scale-105 active:scale-95" style={{ backgroundColor: active ? 'var(--accent)' : '#f4f4f5' }} aria-label={playing ? `Pausar ${station.name}` : `Escuchar ${station.name}`}>
